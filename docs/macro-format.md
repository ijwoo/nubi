...
| `op` | 인자 | 설명 |
| --- | --- | --- |
| `launch` | `url` 또는 `bundleId`, `restart?` | 앱 실행. URL 스킴 우선 |
...
`launch`의 `restart`는 **"앱의 첫 화면부터 시작"**을 뜻한다. 없으면 이미 떠 있는 앱은 마지막으로 보던 화면 그대로 올라온다 — 이어서 할 때는 맞고, 알려진 지점에서 출발해야 할 때는 틀리다.
...
**통계는 실사용에서만 쌓인다.** eval은 측정이지 사용이 아니다 — 벤치마크로 50번 돌리면 그게 히스토리를 지배하고, eval은 일부러 깨진 매크로도 돌린다. `MacroStore.recordRun()`은 라우팅을 통한 실행 경로에서만 호출한다.
